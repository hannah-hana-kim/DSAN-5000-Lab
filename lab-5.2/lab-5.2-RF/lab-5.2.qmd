...
best_split <- function(x, y) {
  ...
  mid <- length_x/2
  left_half <- x[1:mid]
  right_half <- x[mid+1:length(x)]
  
  first_rss <- rss(left_half)
  second_rss <- rss(right_half)
  
  if (first_rss < second_rss) {
    min_rss <- first_rss
  }
  else if (second_rss < first_rss) {
    min_rss <- second_rss
  }
  
  return(list(var1 = mid, var2 = min_rss))
}
```
...
set.seed(123)
# train: 80%, test: 20%
row.number <- sample(1:nrow(abalone), 0.8*nrow(abalone))
train = abalone[row.number,]
test = abalone[-row.number,]
dim(train)
dim(test)
```

```{r}
# data exploring; density function
ggplot(train, aes(Rings)) + geom_density(fill="lightblue")
ggplot(train, aes(log(Rings))) + geom_density(fill="lightblue")
ggplot(train, aes(sqrt(Rings))) + geom_density(fill="lightblue")
```

```{r}
# linear model
model1 = lm(log(Rings)~., data=train)
summary(model1)
par(mfrow=c(2,2))
plot(model1)
```
...
```{r}
# feature selection
# based on the summary result above, we can remove the 'SexM' column since the p-value is big enough to say this column is not significant variable when predicting the target variable. 
model2 <- update(model1, . = model1[, !(names(model1) %in% "SexM")])
model2 = update(model1, ~.-SexM) 
summary(model2) 
```
...
```
```{r}
ggplot(data = airfoil, aes(x = Frequency, y = Pressure)) +
  geom_point(color = "lightblue", shape = 16) +
  labs(x = "Frequency", y = "Pressure", title = "Relationship between frequency and pressure")
```
```{r}
ggplot(airfoil, aes(Pressure)) + geom_density(fill="lightblue")
ggplot(airfoil, aes(log(Pressure))) + geom_density(fill="lightblue")
ggplot(airfoil, aes(sqrt(Pressure))) + geom_density(fill="lightblue")
```
...
set.seed(123)
# train: 80%, test: 20%
row.number <- sample(1:nrow(airfoil), 0.8*nrow(airfoil))
train2 = airfoil[row.number,]
test2 = airfoil[-row.number,]
dim(train2)
dim(test2)
```
...
```{r}
lm_model = lm(log(Pressure)~., data=train2)
summary(lm_model)
par(mfrow=c(2,2))
plot(lm_model)
```
...
```{r}
dt_model <- rpart(data = train2, method = 'class')
rpart.plot(dt_model, box.palette = 'blue')
```
